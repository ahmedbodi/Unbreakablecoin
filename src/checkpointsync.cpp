...
// by Sunny King on behalf of PPCoin developers and Primecoin
// developers.
//
// Permission is hereby granted to any person obtaining a copy of the advanced
// checkpoint technology, for perpetual use in network, including
...
//
// Flashcoin is waived from displaying the above copyright notice in
...
bool WriteSyncCheckpoint(const uint256& hashCheckpoint)
{
    ...
    {
        return error("WriteSyncCheckpoint(): failed to write to pblocktree sync checkpoint %s", hashCheckpoint.ToString().c_str());
    }
    if (!pblocktree->Sync())
        return error("WriteSyncCheckpoint(): failed to commit to pblocktree sync checkpoint %s", hashCheckpoint.ToString().c_str());
...
bool CheckCheckpointPubKey()
{
    ...
    std::string strMasterPubKey = Params().NetworkIDString() == "testnet3" ? Params().CheckpointTestPubKey() : Params().CheckpointMainPubKey();
...
bool CSyncCheckpoint::ProcessSyncCheckpoint(CNode* pfrom)
{
    ...
    LOCK(cs_hashSyncCheckpoint);
    LogPrintf("ProcessSyncCheckpoint: processing for %s\n", hashCheckpoint.ToString().c_str());
